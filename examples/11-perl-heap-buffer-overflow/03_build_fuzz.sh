...
cd perl5
make clean
./Configure -Dprefix=$PWD/build -Dcc=$RACING_DIR/Racing-code/afl-clang-fast -A ccflags="-g -O0 -fsanitize=address -fPIC -poc_trace=$EVAL_DIR/temp_data/poc.addr2line" -Dld=$RACING_DIR/Racing-code/afl-clang-fast -A ldflags="-g -O0 -fsanitize=address" -Dusedevel -d -e -s
make install.perl
